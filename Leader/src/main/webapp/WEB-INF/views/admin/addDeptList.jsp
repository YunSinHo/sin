...
function insertDeptList(){
	if(confirm("등록하시겠습니까?")){
		document.addDeptFrm.action="insertDeptList.mdo";
		document.addDeptFrm.submit();
		return true;
	}
}
function deleteDeptList(id){
	if(confirm("삭제하시겠습니까?")){
		document.addDeptFrm.action="deleteDeptList.mdo?id="+id;
		document.addDeptFrm.submit();
		return true;
	}
}
</script>
<form name="addDeptFrm" method="POST" >
<h2 style="color:black;">부서 추가</h2>
			<c:forEach items="${departmentList}" var="list">
				<input type="text" style="border:none;"
					value="${list.name}">
				&nbsp;
				<button type="button" class="btn btn-danger" onclick="deleteDeptList('${list.id}');">삭제</button><br><br>
			</c:forEach>
...
<button type="button" class="btn btn-success" onclick="insertDeptList();">등록하기</button>
</form>
</body>
</html>